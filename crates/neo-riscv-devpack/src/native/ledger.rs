//! Bindings to the `Ledger` native contract.
//!
//! Wraps block and transaction queries (current hash/index, get block/tx).
use alloc::vec::Vec;
...
/// Native contract script hash for this contract (`LEDGER_CONTRACT_HASH`).
...
/// Return the index (height) of the current block.
