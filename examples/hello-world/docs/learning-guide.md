# hello-world Technical Learning Guide

This guide explains `hello-world` as a Neo N4 technical unit. It is written for architecture learning: what the unit is responsible for, which assumptions make it correct, how data moves, how state changes, how evidence is checked, and where it plugs into the wider Neo N4 stack.
...
| Purpose | Small hello-world contract example for the RISC-V toolchain. |
| Inputs | developer source <br> template runtime <br> test context |
| Responsibilities | Demonstrate contract pattern <br> Exercise tooling <br> Provide learning fixture |
| Outputs | compiled contract <br> example result <br> tutorial artifact |
...
`hello-world` receives developer source | template runtime | test context and owns this boundary: Demonstrate contract pattern | Exercise tooling | Provide learning fixture. It emits compiled contract | example result | tutorial artifact, which are consumed by RISC-V host | Neo N4 L2 node | developer tooling.
