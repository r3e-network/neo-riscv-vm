namespace Neo.SmartContract.RiscV
{
    ...
    {
        // Internal-only facade identity for the NeoVM compatibility contract running on RiscvVM.
        // This is not a user-deployable contract hash and must not leak into contract-visible
        // script-hash semantics. It exists only to make the compatibility route explicit.
        internal static readonly UInt160 NeoVmCompatibilityFacadeHash =
            new(new byte[]
            {
        ...
        {
            return contractType == ContractType.NeoVM ? NeoVmCompatibilityFacadeHash : actualScriptHash;
        }
    }
}
