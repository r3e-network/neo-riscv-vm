//! Bindings to the `Policy` native contract.
//!
//! Wraps fee-per-byte, execution-fee factor, and blocked-account queries.
...
/// Native contract script hash for this contract (`POLICY_CONTRACT_HASH`).
...
/// Invoke the native contract `policy_get_attribute_fee` method.
...
/// Invoke the native contract `policy_get_max_valid_until_block_increment` method.
...
/// Invoke the native contract `policy_get_max_traceable_blocks` method.
