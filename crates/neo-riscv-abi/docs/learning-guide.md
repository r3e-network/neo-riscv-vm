# neo-riscv-abi Technical Learning Guide

This guide explains `neo-riscv-abi` as a Neo N4 technical unit. It is written for architecture learning: what the unit is responsible for, which assumptions make it correct, how data moves, how state changes, how evidence is checked, and where it plugs into the wider Neo N4 stack.
...
| Inputs | shared VM types <br> host/guest boundary <br> serialized stack values |
| Responsibilities | Define stable ABI <br> Re-export shared metadata <br> Encode/decode stack values |
| Outputs | ABI types <br> codec helpers <br> runtime constants |
...
`neo-riscv-abi` receives shared VM types | host/guest boundary | serialized stack values and owns this boundary: Define stable ABI | Re-export shared metadata | Encode/decode stack values. It emits ABI types | codec helpers | runtime constants, which are consumed by RISC-V host | Neo N4 L2 node | developer tooling.
