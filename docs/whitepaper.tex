...
\begin{enumerate}[noitemsep]
    \item \textbf{NeoVM Path}: Existing NeoVM bytecode is executed by the
          shared \texttt{neo-vm-rs} interpreter through the PolkaVM guest
          sandbox boundary. This path preserves complete backward compatibility with
...
\begin{tikzpicture}[
    ...
    \node[below=0.15cm of guest, font=\scriptsize\itshape, color=gray]
        {neo-vm-rs facade (guest.polkavm) \quad$\bullet$\quad Native RISC-V contract binaries};
...
\begin{itemize}[noitemsep]
    \item \textbf{\texttt{neo-vm-rs} Facade} (\texttt{guest.polkavm}): A
          \texttt{no\_std} guest-facing facade over the shared NeoVM semantics.
          It receives NeoVM bytecode via PolkaVM's auxiliary data mechanism
          and communicates with the host through \texttt{host\_call}
          SYSCALLs. Opcode, stack value, syscall metadata, and conversion
          semantics remain centralized in \texttt{neo-vm-rs}.
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Stack Item Codec}: Bidirectional serialization between
          guest \texttt{StackValue} (shared fast codec) and C\# \texttt{StackItem}
          (native struct).
...
\begin{tikzpicture}[
    ...
    \node[box, below=2.0cm of type, xshift=-3cm, fill=neovmblue!10] (neovm)
        {PolkaVM $\to$ neo-vm-rs Facade\\[2pt]{\tiny shared opcodes, stack semantics}};
...
\begin{enumerate}[noitemsep]
    \item \textbf{Postcard Codec} (guest $\leftrightarrow$ host): Stack items
          are serialized using the shared \texttt{neo-vm-rs} fast codec. Tags:
...
\begin{tikzpicture}[
    ...
    \node[box, fill=neovmblue!10] (a1) {neo-vm-rs Facade};
...
\begin{enumerate}[noitemsep]
    \item \textbf{Guest-level}: The shared NeoVM execution path handles
...
Verification runs through the shared NeoVM execution path regardless of the contract
...
bridge. Native contracts use the NeoVM compatibility path exposed through
\texttt{neo-vm-rs} and the PolkaVM boundary.
...
\texttt{neo-riscv-guest} & \texttt{no\_std} facade over shared \texttt{neo-vm-rs} semantics \\
...
\begin{enumerate}
    \item \textbf{Phase 1 --- Bridge Deployment}:
          ...
          contracts execute transparently through the shared \texttt{neo-vm-rs}
