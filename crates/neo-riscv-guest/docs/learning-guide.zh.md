# neo-riscv-guest 技术学习指南

这份指南把 `neo-riscv-guest` 当作 Neo N4 的一个技术单元来解释。它不是源码阅读图，而是帮助读者理解：这个单元负责什么、哪些技术假设保证它正确、数据如何移动、状态如何变化、证据如何被验证、它如何接入 Neo N4 的整体架构。
...
| 输入 | 合约字节码 <br> ABI 栈 <br> syscall stub |
| 职责 | 调用共享 VM runtime <br> 暴露 no_std 合约 API <br> 桥接 syscall |
| 输出 | guest 结果 <br> syscall 请求 <br> 栈变化 |
...
`neo-riscv-guest` 接收 合约字节码 | ABI 栈 | syscall stub，拥有的边界是：调用共享 VM runtime | 暴露 no_std 合约 API | 桥接 syscall。它输出 guest 结果 | syscall 请求 | 栈变化，然后由 RISC-V host | Neo N4 L2 节点 | 开发者工具 消费。
