# neo-riscv-contract-harness 技术学习指南

这份指南把 `neo-riscv-contract-harness` 当作 Neo N4 的一个技术单元来解释。它不是源码阅读图，而是帮助读者理解：这个单元负责什么、哪些技术假设保证它正确、数据如何移动、状态如何变化、证据如何被验证、它如何接入 Neo N4 的整体架构。
...
| 目的 | 用于合约级 RISC-V 执行和 syscall 模拟的测试 harness。 |
| 输入 | 合约模块 <br> mock 上下文 <br> 期望栈 |
| 职责 | 初始化测试上下文 <br> 运行合约 export <br> 断言状态/栈 |
| 输出 | 测试结果 <br> 轨迹 <br> 失败诊断 |
...
`neo-riscv-contract-harness` 接收 合约模块 | mock 上下文 | 期望栈，拥有的边界是：初始化测试上下文 | 运行合约 export | 断言状态/栈。它输出 测试结果 | 轨迹 | 失败诊断，然后由 RISC-V host | Neo N4 L2 节点 | 开发者工具 消费。
