...
这些图都放在本 crate 目录下，用技术架构视角解释 `neo-riscv-contract-harness`。重点是系统位置、技术原理、数据移动、工作流、状态、证明/证据、信任边界、集成关系和运行生命周期。
...
- **输入:** 合约模块 | mock 上下文 | 期望栈
- **职责:** 初始化测试上下文 | 运行合约 export | 断言状态/栈
- **输出:** 测试结果 | 轨迹 | 失败诊断
