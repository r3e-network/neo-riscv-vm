# neo-riscv-guest-module 技术学习指南

这份指南把 `neo-riscv-guest-module` 当作 Neo N4 的一个技术单元来解释。它不是源码阅读图，而是帮助读者理解：这个单元负责什么、哪些技术假设保证它正确、数据如何移动、状态如何变化、证据如何被验证、它如何接入 Neo N4 的整体架构。
...
| 输入 | PolkaVM import <br> guest runtime <br> 编码执行输入 |
| 职责 | 暴露 export <br> 调用 guest runtime <br> 返回编码输出 |
| 输出 | guest.polkavm <br> 编码输出 <br> host callback 调用 |
...
`neo-riscv-guest-module` 接收 PolkaVM import | guest runtime | 编码执行输入，拥有的边界是：暴露 export | 调用 guest runtime | 返回编码输出。它输出 guest.polkavm | 编码输出 | host callback 调用，然后由 RISC-V host | Neo N4 L2 节点 | 开发者工具 消费。
