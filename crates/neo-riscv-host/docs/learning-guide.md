# neo-riscv-host Technical Learning Guide

This guide explains `neo-riscv-host` as a Neo N4 technical unit. It is written for architecture learning: what the unit is responsible for, which assumptions make it correct, how data moves, how state changes, how evidence is checked, and where it plugs into the wider Neo N4 stack.
...
| Inputs | PolkaVM module <br> execution context <br> host syscall provider |
| Responsibilities | Instantiate module <br> Charge opcodes <br> Marshal stack values <br> Return VM result |
| Outputs | execution result <br> gas report <br> host trace |
...
`neo-riscv-host` receives PolkaVM module | execution context | host syscall provider and owns this boundary: Instantiate module | Charge opcodes | Marshal stack values | Return VM result. It emits execution result | gas report | host trace, which are consumed by RISC-V host | Neo N4 L2 node | developer tooling.
