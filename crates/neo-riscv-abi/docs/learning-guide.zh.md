# neo-riscv-abi 技术学习指南

这份指南把 `neo-riscv-abi` 当作 Neo N4 的一个技术单元来解释。它不是源码阅读图，而是帮助读者理解：这个单元负责什么、哪些技术假设保证它正确、数据如何移动、状态如何变化、证据如何被验证、它如何接入 Neo N4 的整体架构。
...
| 输入 | 共享 VM 类型 <br> host/guest 边界 <br> 序列化栈值 |
| 职责 | 定义稳定 ABI <br> 重导出共享元数据 <br> 编码/解码栈值 |
| 输出 | ABI 类型 <br> codec helper <br> 运行时常量 |
...
`neo-riscv-abi` 接收 共享 VM 类型 | host/guest 边界 | 序列化栈值，拥有的边界是：定义稳定 ABI | 重导出共享元数据 | 编码/解码栈值。它输出 ABI 类型 | codec helper | 运行时常量，然后由 RISC-V host | Neo N4 L2 节点 | 开发者工具 消费。
