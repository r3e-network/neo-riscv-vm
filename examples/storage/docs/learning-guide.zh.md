...
这份指南把 `storage` 当作 Neo N4 的一个技术单元来解释。它不是源码阅读图，而是帮助读者理解：这个单元负责什么、哪些技术假设保证它正确、数据如何移动、状态如何变化、证据如何被验证、它如何接入 Neo N4 的整体架构。
...
`storage` 接收 开发者源码 | 模板 runtime | 测试上下文，拥有的边界是：演示合约模式 | 覆盖工具链 | 提供学习 fixture。它输出 编译后合约 | 示例结果 | 教程产物，然后由 RISC-V host | Neo N4 L2 节点 | 开发者工具 消费。
