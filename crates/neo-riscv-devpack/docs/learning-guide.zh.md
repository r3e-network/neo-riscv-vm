# neo-riscv-devpack 技术学习指南

这份指南把 `neo-riscv-devpack` 当作 Neo N4 的一个技术单元来解释。它不是源码阅读图，而是帮助读者理解：这个单元负责什么、哪些技术假设保证它正确、数据如何移动、状态如何变化、证据如何被验证、它如何接入 Neo N4 的整体架构。
...
| 输入 | 合约源码 <br> 模板配置 <br> 工具链设置 |
| 职责 | 构建产物 <br> 校验元数据 <br> 打包部署文件 |
| 输出 | 合约包 <br> manifest <br> 开发诊断 |
...
`neo-riscv-devpack` 接收 合约源码 | 模板配置 | 工具链设置，拥有的边界是：构建产物 | 校验元数据 | 打包部署文件。它输出 合约包 | manifest | 开发诊断，然后由 RISC-V host | Neo N4 L2 节点 | 开发者工具 消费。
