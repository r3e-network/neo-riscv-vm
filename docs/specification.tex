...
\large NeoVM Execution via Shared neo-vm-rs Semantics and PolkaVM Sandbox}
...
\begin{tikzpicture}[
  ...
  \draw[arr, <->] (host) -- node[right, font=\footnotesize] {shared fast codec} (guest);
...
neo-riscv-guest & \texttt{no\_std} & 643 & Facade over shared \texttt{neo-vm-rs} semantics \\
...
\label{thm:isolation}
The shared \texttt{neo-vm-rs} NeoVM execution path is exposed through the PolkaVM RISC-V sandbox boundary. The guest cannot access host memory, execute host system calls, or modify host state except through the defined \texttt{host\_call} protocol.
\end{theorem}
...
\label{thm:equiv}
For the implemented opcode subset, the RISC-V guest module delegates to the shared \texttt{neo-vm-rs} semantics and produces identical results to the C\# NeoVM reference implementation.
\end{theorem}

\begin{proof}
We establish equivalence through three properties:

\textbf{(P1) Opcode semantics identity:} Each opcode implementation in the shared \texttt{neo-vm-rs} interpreter follows the NeoVM specification byte-for-byte:
\begin{itemize}
